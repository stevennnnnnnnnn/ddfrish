...
class OrderPayView(View):
    ...
    def post(self, request):
        ...
        if order.pay_method != 3:
            # 目前只写了支付宝接口
            return JsonResponse({'res': 5, 'errmsg': '目前仅支持支付宝付款，请重新下单'})
...
# /order/buy_now
class OrderBuyNowView(LoginRequiredMixin, View):
    """立即购买页面显示"""
    ...
    def post(self, request):
        ...
        if not sku_id:
            # cart page
            return JsonResponse({'res': 1, 'errmsg': '商品id不正确！'})
